...
static int aipu_release(struct inode *inode, struct file *filp)
{
	...
	/*
	 * release() is the fd-close path; VFS ignores our return value, so
	 * we will not be retried. Whatever cancel_jobs() reports, we MUST
	 * still free this filp's buffers and balance the pm_runtime ref
	 * taken in aipu_open(). The previous "if (ret) return ret" leaked
	 * V3 custom-IOVA buffers on cancel failure -- their accounting in
	 * asid_iova[].iova_used was never decremented, eventually producing
	 * "V3: ASID 0 out of IOVA space" under long-running multi-process
	 * stress -- and also left pm_runtime imbalanced, matching the
	 * observed "PM runtime put failed! ret=-11" warning.
	 */
	ret = aipu_job_manager_cancel_jobs(&aipu->job_manager, filp);
	if (ret)
		dev_warn(aipu->dev,
			 "release: cancel_jobs failed (%d); proceeding with cleanup anyway\n",
			 ret);

	aipu_mm_free_buffers(&aipu->mm, filp);

	/*
	 * Reclaim any dma-bufs this fd attached but never detached. importer
	 * buffers are not tracked in mm->buffer_list, so aipu_mm_free_buffers()
	 * above does not cover them.
	 */
	aipu_detach_dma_buf_by_filp(&aipu->mm, filp);
